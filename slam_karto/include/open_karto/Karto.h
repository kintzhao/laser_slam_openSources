...
namespace karto
{
   ...
   * (x, y) is at bottom left in mapper!左下角
   */
  template<typename T>
  class Rectangle2
  {
  ...
  public:
    ...
    {
      Pose2 deviceOffsetPose2 = GetLaserRangeFinder()->GetOffsetPose();//base 到scan的变换
    ...
    void Update()
    {
      ...
      if (pLaserRangeFinder != NULL)
      {
        ...
        if (nPoints != 0.0)
        {
          ...
          m_BarycenterPose = Pose2(averagePosition, 0.0);  // 端点的均值
        }
        else
        {
  ...
  public:
      ...
      , m_pCellUpdater(NULL)
    {
      ...
      m_pOccupancyThreshold = new Parameter<kt_double>("OccupancyThreshold", 0.1);  //  hit/free  > 0.1
  ...
  protected:
                                  ...
                                  Vector2<kt_double>& rOffset)
    {
      ...
      Size2<kt_double> size = boundingBox.GetSize(); //* Get the size of the bounding box   m_Maximum - m_Minimum
    ...
    virtual kt_bool AddScan(LocalizedRangeScan* pScan, kt_bool doUpdate = false)
    {
      ...
      const_forEachAs(PointVectorDouble, &rPointReadings, pointsIter)
      {
        ...
        kt_bool isEndPointValid = rangeReading < (rangeThreshold - KT_TOLERANCE);//有效点

        if (rangeReading >= maxRange || isnan(rangeReading))//无效
        {
          // ignore max range readings
          pointIndex++;
          continue;
        }
        else if (rangeReading >= rangeThreshold) //  rangeThreshold 与 maxRange  之间
        {
  ...
  public:
                        ...
                        kt_double angleResolution)
    {
      ...
      const_forEach(PointVectorDouble, &rPointReadings)
      {
        ...
        localPoints.push_back(vec);// 创建0角度的位姿列表  激光点   相对传感器中心的数据 local ?
      }
      ...
      for (kt_int32u angleIndex = 0; angleIndex < nAngles; angleIndex++)
      {
        ...
        ComputeOffsets(angleIndex, angle, localPoints);//  将激光数据转换到角度分量的列表中
      }
  ...
  private:
     ...
     * 创建基于角度分量的  pos表的每个表格旋转角度angle建立角度系数的表格.
     * @param angleIndex
     * @param angle
     * @param rLocalPoints
     */
    ...
    {
        // 每个角度对应一个laserscan的列表
      m_ppLookupArray[angleIndex]->SetSize(static_cast<kt_int32u>(rLocalPoints.size()));//m_ppLookupArray
      ...
      kt_int32u readingIndex = 0;
              //m_ppLookupArray[angleIndex]
      kt_int32s* pAngleIndexPointer = m_ppLookupArray[angleIndex]->GetArrayPointer();//地址操作.pAngleIndexPointer结果存到 m_ppLookupArray[angleIndex]中
      ...
      const_forEach(Pose2Vector, &rLocalPoints)
      {
        ...
        pAngleIndexPointer[readingIndex] = lookupIndex;  //pAngleIndexPointer
