...
/**
 * @brief SlamGMapping::initMapper
 * 1. 激光数据解析问题： 正反向   激光角度实行对称形式
 * 2. rangesensor  sensormap
 * 3. 粒子初始化
 * 4. 参数设置，运动模型噪声 扫描匹配 采样
 * @param scan
 * @return
 */
...
SlamGMapping::initMapper(const sensor_msgs::LaserScan& scan)
{
    ...
    tf::Stamped<tf::Pose> ident;            //ident为激光系下点
    ...
    try
    {
        tf_.transformPose(base_frame_, ident, laser_pose);   //laser_pose是基坐标系下的表示
    }
    catch(tf::TransformException e)
    {
    ...
    tf::Stamped<tf::Vector3> up(v, scan.header.stamp,base_frame_);  //up是基坐标系下的点
    try
    {
        tf_.transformPoint(laser_frame_, up, up);      //up转回激光系下
    ...
    if (fabs(fabs(up.z()) - 1) > 0.001)     //判断安装是否水平。   对齐。 z轴的偏转
    {
    ...
    gsp_laser_beam_count_ = scan.ranges.size();// 激光点的数量

    double angle_center = (scan.angle_min + scan.angle_max)/2;

    if (up.z() > 0) //  激光向上安装
    {
        do_reverse_range_ = scan.angle_min > scan.angle_max;  // 解析激光数据是否反转的标志
    ...
    double theta = - std::fabs(scan.angle_min - scan.angle_max)/2;   //角度对称表示××××××
    for(unsigned int i=0; i<scan.ranges.size(); ++i)
    {
        laser_angles_[i]=theta;
        theta += std::fabs(scan.angle_increment);  //laser_angles_  激光角度值×××××
    }
    ...
    //创建sensor数据格式
    gsp_laser_ = new GMapping::RangeSensor("FLASER",//name
                                           gsp_laser_beam_count_,
                                           fabs(scan.angle_increment),  //正角度增量
                                           gmap_pose,
                                           0.0,
                                           maxRange_);
    ...
    gsp_->setSensorMap(smap);//Construct the angle table for the sensor
    ...
    if(!getOdomPose(initialPose, scan.header.stamp))// centered_laser_pose_ 在odom_frame
    {
    ...
    gsp_->setMatchingParameters(maxUrange_, maxRange_, sigma_,
                                ...
                                lsigma_, ogain_, lskip_); //scan matcher param

    gsp_->setMotionModelParameters(srr_, srt_, str_, stt_); //运动模型噪声参数 <gmapping/gridfastslam/motionmodel.h>
    gsp_->setUpdateDistances(linearUpdate_, angularUpdate_, resampleThreshold_);//更新触发设置
    gsp_->setUpdatePeriod(temporalUpdate_);//更新频率
    gsp_->setgenerateMap(false);//generate an accupancy grid map
    gsp_->GridSlamProcessor::init(particles_, xmin_, ymin_, xmax_, ymax_,//ScanMatcherMap lmap
                                  delta_, initialPose);//初始化×××××××××××  m_particles 粒子初始化：pose+lmap

    gsp_->setllsamplerange(llsamplerange_);  //采样参数设置
    ...
    GMapping::sampleGaussian(1,seed_);  //高斯分布
...
{
    if(!getOdomPose(gmap_pose, scan.header.stamp)) //     (odom_frame_, centered_laser_pose_, odom_pose);
        return false;
...
{
    laser_count_++;
    if ((laser_count_ % throttle_scans_) != 0)   //throttle_scans_
        return;

    static ros::Time last_map_update(0,0);

    // We can't initialize the mapper until we've got the first scan
    if(!got_first_scan_)//got_first_scan_
    {
        if(!initMapper(*scan))
            return;
    ...
    if(addScan(*scan, odom_pose))
    {
        ...
        if(!got_map_ || (scan->header.stamp - last_map_update) > map_update_interval_)  //map_update_interval_   地图更新速率
        {
            updateMap(*scan);
...
SlamGMapping::updateMap(const sensor_msgs::LaserScan& scan)
{
    ...
    // the map may have expanded, so resize ros message as well 地图尺寸
    ...
    for(int x=0; x < smap.getMapSizeX(); x++) //占据设置
    {
