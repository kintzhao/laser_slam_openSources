...
ScanMatcher::ScanMatcher(): m_laserPose(0,0,0){
    ...
    m_linearOdometryReliability=0.; //？？？可靠性
...
void ScanMatcher::computeActiveArea(ScanMatcherMap& map, const OrientedPoint& p, const double* readings){
	...
	if ( !map.isInside(min)	|| !map.isInside(max)){
        ...
        map.resize(min.x, min.y, max.x, max.y);   //地图大小调整
...
double ScanMatcher::registerScan(ScanMatcherMap& map, const OrientedPoint& p, const double* readings){
	...
	for (const double* r=readings+m_initialBeamsSkip; r<readings+m_laserBeams; r++, angle++)
		if (m_generateMap){
			...
			GridLineTraversalLine line;
			line.points=m_linePoints;
            GridLineTraversal::gridLine(p0, p1, &line);   //line
			for (int i=0; i<line.num_points-1; i++){
...
/**
 * @brief ScanMatcher::optimize
 *  梯度迭代找最匹配点
 * 先从粒子的位置（运动预测） 开始搜索，迭代结束的条件是次数和分数
 * 每次大迭代找局部最近点 采取的是线位移与角度位移的分度迭代
 *
 * @param pnew  返回 scanMatcher
 * @param map    当前粒子的地图
 * @param init     初始位置 //？？
 * @param readings    当前读取的激光束
 * @return       匹配的分数
 */
...
//		cout <<  "pose=" << currentPose.x  << " " << currentPose.y << " " << currentPose.theta << endl;
		...
		do {
			...
			if (m_angularOdometryReliability>0.){
                ...
                odo_gain*=exp(-m_angularOdometryReliability*dth);   // 调整的高斯均值
			}
