...
bool GridSlamProcessor::processScan(const RangeReading & reading, int adaptParticles){//adaptParticles default 0
    
    /**retireve the position from the reading, and compute the odometry*/
    OrientedPoint relPose=reading.getPose();  //  centered_laser_pose_在odom_frame_的表示
    if (!m_count){
    ...
    // if the robot jumps throw a warning  估计的运动量过大
    ...
    if (! m_count
            || m_linearDistance>=m_linearThresholdDistance   //** 相邻转移量？？
            ...
            || (period_ >= 0.0 && (reading.getTime() - last_update_time_) > period_))
    {
        ...
        if (m_count>0)   //scanMatch*******************************
        {
            scanMatch(plainReading);  //优化算法调整粒子的位置，同时进行权重更新
            if (m_outputStream.is_open()){
            ...
            onScanmatchUpdate();//选最优的粒子更新gui中的地图
        ...
        updateTreeWeights(false);    //TreeWeights  ???
        //		cerr  << ".done!" <<endl;
